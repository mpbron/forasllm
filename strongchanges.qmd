--- 
title: LLM False Positives (Only changes from *No* to *Yes*) - FORAS
author: 
    name: Michiel Bron
...
selection =  [(k, num_of_exclusions(ds_env.truth[k])) for k,strong_change in strong_changes.items() if strong_change]
...
for n_ex in sorted_exclusions:
    keys = selection_dict[n_ex]
    selection_prov = ds_env.create_bucket(keys)
    ds_env.to_pandas(selection_prov, labels=pred_provider, label_viewer=symbol_label_viewer()).to_excel(f"output/export_{n_ex}.xlsx")
```